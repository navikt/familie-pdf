...
private fun htmlElements(htmlString: String) =
    HtmlConverter.convertToElements(htmlString, ConverterProperties())

private fun blockElement(blockElement: IBlockElement): IBlockElement {
    return when (blockElement) {
        is List -> {
            ...
            blockElement.children.forEach { child ->
                if (child is ListItem) {
                    ...
                    child.children.forEach { subChild ->
                        if (subChild is IBlockElement) newItem.add(blockElement(subChild))
...
private fun textElement(txt: ILeafElement): ILeafElement {
    if (txt is Text) {
        ...
        // Check if this Text is actually a link
